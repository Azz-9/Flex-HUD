...
final class ModifierEditorPopup {
	...
	boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
		for (PopupTextFieldWidget field : parameterFields) {
			if (field.mouseDragged(mouseX, mouseY, button, deltaX, deltaY)) {
				return true;
			}
		}
		for (ConditionalBranchRow row : conditionalRows) {
	...
	boolean charTyped(char chr, int modifiers) {
		for (PopupTextFieldWidget field : parameterFields) {
			if (field.isFocused() && field.charTyped(chr, modifiers)) {
				return true;
			}
		}
		for (ConditionalBranchRow row : conditionalRows) {
	...
	private final class ConditionalBranchRow {
		...
		private boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
			return thresholdField.mouseDragged(mouseX, mouseY, button, deltaX, deltaY)
					|| resultField.mouseDragged(mouseX, mouseY, button, deltaX, deltaY);
		}
		...
		private boolean charTyped(char chr, int modifiers) {
			return (thresholdField.isFocused() && thresholdField.charTyped(chr, modifiers))
					|| (resultField.isFocused() && resultField.charTyped(chr, modifiers));
		}
