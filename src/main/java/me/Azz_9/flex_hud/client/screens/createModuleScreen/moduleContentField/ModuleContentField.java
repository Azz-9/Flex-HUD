...
public class ModuleContentField extends AbstractWidget implements TrackableChange {
	...
	private final class ColorPopup {
		...
		private void layout(SelectionBounds selectionBounds) {
			...
			int headerWidth = textButtonWidth(Component.translatable("flex_hud.create_module_screen.editor.no_color"))
			...
			noneBounds = new Bounds(buttonX, bounds.y() + POPUP_PADDING, textButtonWidth(Component.translatable("flex_hud.create_module_screen.editor.no_color")), BUTTON_HEIGHT);
			buttonX = noneBounds.right() + POPUP_GAP;
			chromaBounds = new Bounds(buttonX, bounds.y() + POPUP_PADDING, textButtonWidth(Component.translatable("flex_hud.create_module_screen.editor.chroma")), BUTTON_HEIGHT);
			closeBounds = new Bounds(bounds.right() - POPUP_PADDING - textButtonWidth(Component.translatable("flex_hud.global.config.cancel")), bounds.y() + POPUP_PADDING, textButtonWidth(Component.translatable("flex_hud.global.config.cancel")), BUTTON_HEIGHT);
		...
		private void render(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float deltaTicks) {
			renderPanel(graphics, bounds);
			renderButtonCenterLabel(graphics, noneBounds, Component.translatable("flex_hud.create_module_screen.editor.no_color"), noneBounds.contains(mouseX, mouseY) ? BUTTON_HOVERED_BACKGROUND : BUTTON_BACKGROUND, BUTTON_TEXT_COLOR, mouseX, mouseY);
			renderButtonCenterLabel(graphics, chromaBounds, Component.translatable("flex_hud.create_module_screen.editor.chroma"), chromaBounds.contains(mouseX, mouseY) ? BUTTON_HOVERED_BACKGROUND : BUTTON_BACKGROUND, BUTTON_TEXT_COLOR, mouseX, mouseY);
			renderButtonCenterLabel(graphics, closeBounds, Component.translatable("flex_hud.global.config.cancel"), closeBounds.contains(mouseX, mouseY) ? BUTTON_HOVERED_BACKGROUND : BUTTON_BACKGROUND, BUTTON_TEXT_COLOR, mouseX, mouseY);

			for (int i = 0; i < presetBounds.size(); i++) {
				Bounds swatch = presetBounds.get(i);
				int color = PRESET_COLORS.get(i);
				graphics.fill(swatch.x(), swatch.y(), swatch.right(), swatch.bottom(), 0xff000000 | color);
				graphics.outline(swatch.x(), swatch.y(), swatch.width(), swatch.height(), POPUP_BORDER);
			}
		...
		private boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
			...
			for (int i = 0; i < presetBounds.size(); i++) {
				if (presetBounds.get(i).contains(event.x(), event.y())) {
		...
		private int textButtonWidth(Component text) {
			return MINECRAFT.font.width(text) + BUTTON_HORIZONTAL_PADDING * 2;
		}
	...
	private final class GradientPopup {
		...
		private void layout(SelectionBounds selectionBounds) {
			...
			clearBounds = new Bounds(bounds.right() - POPUP_PADDING - textButtonWidth(Component.translatable("flex_hud.create_module_screen.editor.no_color")) - POPUP_GAP - textButtonWidth(Component.translatable("flex_hud.global.config.cancel")), bounds.y() + POPUP_PADDING, textButtonWidth(Component.translatable("flex_hud.create_module_screen.editor.no_color")), BUTTON_HEIGHT);
			closeBounds = new Bounds(bounds.right() - POPUP_PADDING - textButtonWidth(Component.translatable("flex_hud.global.config.cancel")), bounds.y() + POPUP_PADDING, textButtonWidth(Component.translatable("flex_hud.global.config.cancel")), BUTTON_HEIGHT);
		...
		private void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float deltaTicks) {
			renderPanel(graphics, bounds);
			renderButtonCenterLabel(graphics, startBounds, Component.translatable("flex_hud.create_module_screen.editor.gradient_start"), startBounds.contains(mouseX, mouseY) || editingStart ? BUTTON_HOVERED_BACKGROUND : BUTTON_BACKGROUND, BUTTON_TEXT_COLOR, mouseX, mouseY);
			renderButtonCenterLabel(graphics, endBounds, Component.translatable("flex_hud.create_module_screen.editor.gradient_end"), endBounds.contains(mouseX, mouseY) || !editingStart ? BUTTON_HOVERED_BACKGROUND : BUTTON_BACKGROUND, BUTTON_TEXT_COLOR, mouseX, mouseY);
			renderButtonCenterLabel(graphics, clearBounds, Component.translatable("flex_hud.create_module_screen.editor.no_color"), clearBounds.contains(mouseX, mouseY) ? BUTTON_HOVERED_BACKGROUND : BUTTON_BACKGROUND, BUTTON_TEXT_COLOR, mouseX, mouseY);
			renderButtonCenterLabel(graphics, closeBounds, Component.translatable("flex_hud.global.config.cancel"), closeBounds.contains(mouseX, mouseY) ? BUTTON_HOVERED_BACKGROUND : BUTTON_BACKGROUND, BUTTON_TEXT_COLOR, mouseX, mouseY);
