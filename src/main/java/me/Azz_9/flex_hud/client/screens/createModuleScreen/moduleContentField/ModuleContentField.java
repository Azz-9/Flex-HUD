...
import me.Azz_9.flex_hud.client.utils.ScissorUtils;
...
public class ModuleContentField extends ClickableWidget implements TrackableChange {
	...
	protected void renderWidget(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
		...
		context.drawGuiTexture(RenderLayer::getGuiTextured, texture, getX(), getY(), getWidth(), getHeight());
	...
	private void renderContent(DrawContext context, int innerLeft, int contentTextY) {
		for (DisplayItem item : displayItems) {
			...
			if (item instanceof ConditionDisplayItem conditionDisplayItem) {
				...
				drawBorder(context, drawX, chipTop, conditionDisplayItem.width(), conditionDisplayItem.height(), CONDITION_BORDER_COLOR);
			...
			drawBorder(context, drawX, chipTop, variableDisplayItem.width(), variableDisplayItem.height(), VARIABLE_BORDER_COLOR);
	...
	private void renderTooltip(DrawContext context, int mouseX, int mouseY) {
		...
		ScreenRect currentScissor = ScissorUtils.peekLast(context);
	...
	public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
		if (gradientPopup != null && gradientPopup.mouseDragged(mouseX, mouseY, button, deltaX, deltaY)) {
			return true;
		}
		if (colorPopup != null && colorPopup.mouseDragged(mouseX, mouseY, button, deltaX, deltaY)) {
			return true;
		}
		if (modifierEditorPopup != null && modifierEditorPopup.mouseDragged(mouseX, mouseY, button, deltaX, deltaY)) {
			return true;
		}
		if (conditionEditorPopup != null && conditionEditorPopup.mouseDragged(mouseX, mouseY, button, deltaX, deltaY)) {
			return true;
		}
		if (modifierPickerPopup != null && modifierPickerPopup.mouseDragged(mouseX, mouseY, button, deltaX, deltaY)) {
			return true;
		}
	...
	public boolean charTyped(char chr, int modifiers) {
		...
		if (gradientPopup != null && gradientPopup.charTyped(chr, modifiers)) {
			return true;
		}
		if (colorPopup != null && colorPopup.charTyped(chr, modifiers)) {
			return true;
		}
		if (modifierEditorPopup != null && modifierEditorPopup.charTyped(chr, modifiers)) {
			return true;
		}
		if (conditionEditorPopup != null && conditionEditorPopup.charTyped(chr, modifiers)) {
			return true;
		}
		if (modifierPickerPopup != null && modifierPickerPopup.charTyped(chr, modifiers)) {
			return true;
		}
		...
		switch (keyCode) {
			...
			case GLFW.GLFW_KEY_LEFT -> {
				if (Screen.hasControlDown()) {
					setCaret(getWordSkipPosition(-1), Screen.hasShiftDown());
				} else {
					moveCaret(-1, Screen.hasShiftDown());
				}
				return true;
			}
			...
			case GLFW.GLFW_KEY_HOME -> {
				setCaret(0, Screen.hasShiftDown());
				return true;
			}
			case GLFW.GLFW_KEY_END -> {
				setCaret(model.size(), Screen.hasShiftDown());
				return true;
			}
			default -> {
				return false;
	...
	void renderButtonCenterLabel(DrawContext context, Bounds bounds, Text label, int backgroundColor, int textColor, double mouseX, double mouseY) {
		if (!label.getString().isEmpty()) {
			...
			drawBorder(context, bounds.x(), bounds.y(), bounds.width(), bounds.height(), POPUP_BORDER);
	...
	private final class ColorPopup {
		...
		private void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
			...
			for (int i = 0; i < presetBounds.size(); i++) {
				...
				drawBorder(context, swatch.x(), swatch.y(), swatch.width(), swatch.height(), POPUP_BORDER);
			}
		...
		private boolean mouseClicked(double mouseX, double mouseY, int button) {
			...
			for (int i = 0; i < presetBounds.size(); i++) {
				if (presetBounds.get(i).contains(mouseX, mouseY)) {
		...
		private boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
			return selector.mouseDragged(mouseX, mouseY, button, deltaX, deltaY);
		}
		...
		private boolean charTyped(char chr, int modifiers) {
			return selector.charTyped(chr, modifiers);
		}
