...
public class Variables {
	...
	private static void registerPlayerVariables() {
		register("player.gamemode", SafeSupplier.create(() -> requireNonNull(CLIENT.interactionManager).getCurrentGameMode().getSimpleTranslatableName().getString(), Text.translatable("selectWorld.gameMode.survival").getString()), TICK);
	...
	private static void registerClientVariables() {
		...
		register("client.version", () -> SharedConstants.getGameVersion().getId(), ON_JOIN_WORLD);
	...
	private static double scaledNetherCoordinate(double coordinate) {
		World world = requireNonNull(CLIENT.player).getWorld();
