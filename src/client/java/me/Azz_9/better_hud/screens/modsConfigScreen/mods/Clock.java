...
public class Clock extends ModsConfigTemplate {
	...
	protected void init() {
		...
		addToggleButton(getCenterX(), startY, getButtonWidth(), getButtonHeight(), Text.of("Show clock"), INSTANCE.clock.enabled, true,
				toggled -> INSTANCE.clock.enabled = toggled);
		addToggleButton(getCenterX(), startY + 30, getButtonWidth(), getButtonHeight(), Text.of("Text shadow"), INSTANCE.clock.shadow, true,
				toggled -> INSTANCE.clock.shadow = toggled);

		addColorButton(getCenterX(), startY + 60, getButtonWidth(), Text.of("Text color"), INSTANCE.clock.color, 0xffffff,
				color -> INSTANCE.clock.color = color);

		addToggleButton(getCenterX(), startY + 90, getButtonWidth(), getButtonHeight(), Text.of("24-hour format"), INSTANCE.clock.isTwentyFourHourFormat, true,
				toggled -> INSTANCE.clock.isTwentyFourHourFormat = toggled);

		addTextField(getCenterX(), startY + 120, getButtonWidth(), getButtonHeight(), 80, Text.of("Text format"), INSTANCE.clock.textFormat, "hh:mm:ss",
