...
public class CalculateSpeed {
	...
	public static void calculateSpeed(PlayerEntity player) {
		...
		if (previousPosition != null) {
			...
			if (ModConfig.getInstance().speedometer.units == KNOT || (ModConfig.getInstance().speedometer.useKnotInBoat && player.getVehicle() instanceof BoatEntity)) {
				speed = speed * 1.9438452492;
			} else if (ModConfig.getInstance().speedometer.units == KPH) {
				speed = speed * 3.6;
			} else if (ModConfig.getInstance().speedometer.units == MPH) {
