...
public class MovableWidget extends AbstractWidget.WithInactiveMessage implements TrackableChange {
	...
	public boolean keyPressed(@NonNull KeyEvent input) {
		...
		if (this.isFocused() && isArrowKey(input)) {
			...
			pressedKeys.add(input.keycode());
			if (pressedKeys.contains(InputConstants.KEYCODE_UP)) {
				moveTo(getX(), getY() - 1);
			}
			if (pressedKeys.contains(InputConstants.KEYCODE_DOWN)) {
				moveTo(getX(), getY() + 1);
			}
			if (pressedKeys.contains(InputConstants.KEYCODE_LEFT)) {
				moveTo(getX() - 1, getY());
			}
			if (pressedKeys.contains(InputConstants.KEYCODE_RIGHT)) {
				moveTo(getX() + 1, getY());
			}
			updateScaleHandle();
			return true;
