...
import me.Azz_9.flex_hud.client.binding.BoolBinding;
...
import me.Azz_9.flex_hud.client.modules.hud.*;
import me.Azz_9.flex_hud.client.modules.hud.renderable.MultiRenderable;
import me.Azz_9.flex_hud.client.modules.hud.renderable.RenderableItem;
import me.Azz_9.flex_hud.client.modules.hud.renderable.RenderableText;
...
public class ArmorStatus extends AbstractTextModule {
	...
	private int drawItemStack(ItemStack stack, int x, int y, int index) {
		...
		if (PlaceholderStacks.of(stack.getItem()).isDamageableItem()) {
	...
	private void drawArrowsStacks(int x, int y, int horizontalGap, int verticalGap) {
		if (separateArrowTypes.getValue()) {
			...
			for (int i = 0; i < arrows.length; i++) {
				...
				if (CommonClass.isEditingLayout || MINECRAFT.player == null) {
					text = String.valueOf(PlaceholderStacks.of(arrow).getMaxStackSize());
				} else {
