...
public class ToggleSneak extends AbstractTextModule {
	...
	private static @NotNull String getStatusMessage(@Nullable LocalPlayer player) {
		...
		if (CommonClass.isEditingLayout || player == null) {
			...
			boolean sneakKeyPressed = MINECRAFT.options.keyShift.isDown();
			boolean sneakToggled = MINECRAFT.options.toggleCrouch().get();

			if (sneakToggled) {
				if (InputConstants.isKeyDown(MINECRAFT.getWindow().getWindow(), ((KeyMappingAccessor) MINECRAFT.options.keyShift).getBoundKey().getValue())) {
